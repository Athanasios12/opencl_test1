...
__kernel void viterbi_gpu_fragment(	__global const unsigned char *img,
									__global float *L,
									__global int *line_x,
									__global float* V_1,
									__global float* V_2,
...
{
	int start_column = get_global_id(0); //start_column - column from global buffer, but data can be bigger than global size
	long L_id = img_height * L_width * start_column;
	int V_id = img_height *start_column;
	int global_size = get_global_size(0);
	...
	for (; n > start_column; n--)
	{
		x_n = x_n + L[L_id + (x_n * L_width) + (n - 1)]; //L[L_id][row][column]
	}
	// save only last pixel position
	line_x[start_column + first_col_linex] = x_n;
}
