...
bool readConfig(bool debugMode, TestSettings &settings, Algorithm algType)
{
									...
									for (xml_attribute<> *id = columnPtr->first_attribute(); id; id = id->next_attribute())
									{
										...
										if (id_name.compare("id") == 0)
										{
											if ((static_cast<int>(algType) & id_val) != 0 || id_val == 0)
											{
                                                ...
                                                if (id_val > 0)
                                                {
                                                    settings.algorithms.push_back(static_cast<Algorithm>(id_val));
                                                }
											}
										}
									}
    ...
    print("Couldn't open file " << CONFIG_FILE);
	return false;
}
...
int initializeCL(cl_command_queue &command_queue, cl_context &context, cl_device_id &device_id)
{
	...
	if (CL_SUCCESS == err)
	{
		print("Detected OpenCl platforms :" << PlatformRoleMaximum);
	}
	else
	{
		print("Error calling get platforms. error code :" << err);
		getchar();
		return 0;
	}
	...
	{
		print("Error calling getdeviceIds. error code : " << err);
		return err;
	}
	...
	{
		print("Error creating context. error code : " << err);
		return err;
	}
	...
	if (CL_SUCCESS == err)
	{
		print("Created command queue\n");
	}
	else
	{
		print("Error creating command queue. error code : " << err);
		return err;
	}
	return CL_SUCCESS;
}
...
bool test_viterbi(const TestSettings &settings, PlotInfo &plotInfo)
{
	...
	{
        cout << "\nProcessing file " << img_file << " ..." << endl;
        clock_t start_img = clock();
		...
		while (g_high + abs(g_low) > 0 && g_low <= 0 && g_high >= 0) // maybe redundant but whatever...
		{
            ...
            for (auto && algorithm : settings.algorithms)
            {
                if (algorithm & SERIAL || algorithm == ALL)
                {
                    start = clock();
                ...
                if (algorithm & CPP11_THREADS || algorithm == ALL)
                {
                    start = clock();
                    ...
                    line_results.push_back(line_x);
                    times.push_back(time_ms);
                    print("\nThreads C++11 time :" + std::to_string(time_ms));
                }

                //cpu multithread version
                if (algorithm & OPENMP_THREADS || algorithm == ALL)
                {
                    start = clock();
                    viterbi.viterbiOpenMP(line_x, g_low, g_high);
                    end = clock();
                    time_ms = (double)(end - start);
                    detectionError.push_back(checkDetectionError(line_x, test_lines[img_num]));
                    line_results.push_back(line_x);
                    times.push_back(time_ms);
                    print("\nThreads OpenMP time :" + std::to_string(time_ms));
                }

                //viterbi parallel cols gpu version 
                if (algorithm & GPU || algorithm == ALL)
                {
                    start = clock();
                ...
                //hybrid
                if (algorithm & CPP11_HYBRID || algorithm == ALL)
                {
                    start = clock();
                    viterbi.launchHybridViterbi(line_x, g_low, g_high);
                    end = clock();
                    ...
                    line_results.push_back(line_x);
                    times.push_back(time_ms);
                    print("\nHybrid C++11 time :" + std::to_string(time_ms));
                }

                if (algorithm & OPENMP_HYBRID || algorithm == ALL)
                {
                    start = clock();
                    viterbi.launchHybridViterbiOpenMP(line_x, g_low, g_high);
                    end = clock();
                    time_ms = (double)(end - start);
                    line_results.push_back(line_x);
                    times.push_back(time_ms);
                    print("\nHybrid OpenMP time :" + std::to_string(time_ms));
                }
                if (algorithm == ALL)
                {
                    break;
        ...
        cout << "\nFile  " << img_file << " processed in " << clock() - start_img << " ms";
		++img_num;
	}
	//cleanup
	int err = 0;
...
int basicTest(const TestSettings &settings)
{
    ...
    printf("Settings file names:\n");
    for (auto && file : settings.img_names)
    {
        cout << file << endl;
    }
